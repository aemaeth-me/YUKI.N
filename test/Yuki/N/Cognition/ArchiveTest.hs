...
cognitionTaskArchivePersistence =
 ...
 where
  ...
  exercise dir store =
    taskArchiveAppend store running
      >>= withTextRight
        ( \_ ->
            taskArchiveAppend store completed
              >>= withTextRight
                ( \sealed ->
                    taskArchiveAppend store completed
                      >>= withTextRight
...
cognitionTaskArchiveRetrieval =
  newMemoryBlobStore >>= \blobs ->
    newMemoryTaskArchiveStore blobs >>= \store ->
      taskArchiveAppend store primary
        >>= withTextRight
          ( \_ ->
              taskArchiveAppend store secondary
                >>= withTextRight
                  ( \_ ->
                      taskArchiveAppend store otherRun
                        >>= withTextRight
                          ( \_ ->
                              verify store
                          )
