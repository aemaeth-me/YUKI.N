# YUKI.N 前端重设计：YUKI 本位

> 界面文案约定：面向用户只称 `YUKI`、`这位 YUKI` 或具体名字；`Incarnation`
> 保留为领域类型，不再把任何一位称作另一位的「分身」。
...
> **YUKI.N 容纳一位位持续存在、各自记得、各自成形、能工作也能反省的 YUKI。**

因此：

```text
YUKI / Incarnation       最高持久主体
├── Self                 它是谁
├── Memory               它因何成为现在的自己
...
任务项始终可切换。若当前 Run 尚未结束，切换会先中止该 Run，再打开目标任务；
已经保存的记录仍可继续。
