...
newImpressionStore dir =
  createDirectoryIfMissing True dir
    *> loadStore (storePath dir)
    >>= traverse (newMVar >=> pure . mkStore (atomicEncodeFile (storePath dir)))
...
consolidationPromptRevision = "impression-consolidation/v2"
...
consolidateImpression models journal store incarnation experienceRef allowedArchiveRefs experience =
      ...
      let invocationId' = invocationIdentifier "consolidate" incarnation experienceRef (impressionRevision before)
          spec =
            InvocationSpec
              ...
              consolidationPromptRevision
              models
              (consolidationPrompt before allowedArchiveRefs experience)
              2
              24000
              60000
              journal
    ...
    finish before now invocationId' result =
      case
          parseConsolidation
            ...
            (Set.fromList (impressionItems before >>= impressionSourceMemoryIds))
            now
...
consolidationPrompt :: ImpressionState -> [Text] -> Text -> [ChatMessage]
consolidationPrompt state archiveRefs experience =
  [ ChatSystem
      ...
      \remove noise, and do not write prompts or execute tools. The source is the committed Task \
      \Archive / Experience closure and any explicit memory_grep then bounded memory_read evidence \
      \recorded there. sourceMemoryIds is a legacy JSON field: preserve grounded Task/archive refs \
      \already present, or leave it empty; never invent an id. New memoryProposals and voidProposals \
      \are forbidden and both arrays must be empty.",
    ChatUser
      ( Text.intercalate
          "\n\n"
...
parseConsolidation :: Set Text -> Set Text -> Integer -> Text -> Either Text ConsolidationDecision
parseConsolidation allowed legacy now raw =
  ...
  where
    validate decision
      ...
      | any (unknownArchiveRef (Set.union allowed legacy)) (decisionItems decision) = Left "impression references an unknown Task archive catalog id"
