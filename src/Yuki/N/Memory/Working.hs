...
mkStore persist lock =
 ...
 where
  create incarnation cursor
    ...
    | otherwise =
        stampedModifyMVar lock $ \stamp state ->
    ...
    mutateHead persist lock incarnation expected $ \now state head' ->
      stateRequired WorkingAwake head'
        *> advance (workingMemoryCursor head') cursor
        <&> advancedHead state head' now
   where
    advancedHead state head' now advanced =
      if advanced
        then
          let changedHead =
                head'
    ...
    stampedModifyMVar lock $ \stamp state ->
      case Map.lookup incarnation (stateHeads state) of
        ...
        Just head'
          ...
          | otherwise ->
              let cycle' = recoveryCycle state head'
...
stampedModifyMVar :: MVar state -> (Integer -> state -> IO (state, value)) -> IO value
stampedModifyMVar var change =
  getPOSIXTime >>= modifyMVar var . change . round
  ...
  stampedModifyMVar lock $ \stamp state ->
    case Map.lookup incarnation (stateHeads state) of
      ...
      Just head'
        | workingMemoryRevision head' /= expected -> pure (state, Left (stale expected (workingMemoryRevision head')))
        | otherwise ->
            case change stamp state head' of
              Left failure -> pure (state, Left failure)
